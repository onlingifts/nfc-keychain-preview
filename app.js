...
function removeWhiteBackground(c,x){
  const p=x.getImageData(0,0,c.width,c.height),d=p.data;
  for(let i=0;i<d.length;i+=4){
    const r=d[i],g=d[i+1],b=d[i+2];
    const mx=Math.max(r,g,b),mn=Math.min(r,g,b);
    const neutral=mx-mn<30;
    if(neutral&&mx>218){
      const a=Math.max(0,Math.min(255,(240-mx)*12));
...
function processLogo(){
  ...
  img.onload=()=>{
    ...
    if(removeBg.checked)removeWhiteBackground(c,x);
...
function makeDilatedMask(img,w,h,radius){
  const m=document.createElement('canvas');m.width=w;m.height=h;
  const mx=m.getContext('2d');
  mx.fillStyle='#000';
  for(let dy=-radius;dy<=radius;dy+=3){
    for(let dx=-radius;dx<=radius;dx+=3){
      if(dx*dx+dy*dy<=radius*radius)mx.drawImage(img,dx,dy,w,h);
    }
  }
  return m;
}
