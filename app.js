...
function floodRemoveBackground(c,x){
  ...
  const baseThreshold=34;
  const localThreshold=28;
  while(qh<qt){
    ...
    for(const np of neighbors){
      ...
      const ni=np*4;
      const local=colorDist(d,ii,ni);
      let edgeRef=0;
      if(px<3||py<3||px>w-4||py>h-4) edgeRef=baseThreshold+18;
      else edgeRef=baseThreshold;
      if(local<=localThreshold || local<=edgeRef){visited[np]=1;queue[qt++]=np;}
    }
  }
...
function imageComplexity(c){
  ...
  for(let y=step;y<c.height;y+=step){
    for(let xx=step;xx<c.width;xx+=step){
      ...
      const j=(y*c.width+Math.max(0,xx-step))*4;
      if(d[j+3]<30)continue;
      const dr=Math.abs(d[i]-d[j]),dg=Math.abs(d[i+1]-d[j+1]),db=Math.abs(d[i+2]-d[j+2]);
      diffs+=dr+dg+db;samples++;
    }
  }
...
function processLogo(){
  ...
  img.onload=()=>{
    ...
    const ox=original.getContext('2d',{willReadFrequently:true});
    ox.drawImage(img,0,0,original.width,original.height);

    const cleaned=document.createElement('canvas');
    cleaned.width=original.width;cleaned.height=original.height;
    const cx=cleaned.getContext('2d',{willReadFrequently:true});
    cx.drawImage(original,0,0);
    floodRemoveBackground(cleaned,cx);
    const cropped=cropToContent(cleaned);
    const badge=shouldUseBadge(cropped.stats,cropped.canvas);

    renderAll({
      cleanData:cropped.canvas.toDataURL('image/png'),
      originalData:original.toDataURL('image/png'),
      badge
    });
  };
...
function renderAll(result){
  ...
  profileLogo.src=result.badge?result.originalData:result.cleanData;
  profileLogo.style.display='block';
  profilePlaceholder.style.display='none';
  if(result.badge)drawBadgeKeychain(result.originalData);else drawFreeformKeychain(result.cleanData);
}
...
function drawBadgeKeychain(data){
  ...
  img.onload=()=>{
    ...
    const x=(canvas.width-size)/2,y=(canvas.height-size)/2+8;
    const r=size*.14;
    ...
    ctx.save();roundRectPath(ctx,x,y,size,size,r);ctx.clip();ctx.fillStyle='#f2eee7';ctx.fillRect(x,y,size,size);
    const s=Math.max(size/img.width,size/img.height);const w=img.width*s,h=img.height*s;ctx.drawImage(img,x+(size-w)/2,y+(size-h)/2,w,h);ctx.restore();
